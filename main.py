...
import numpy as np
from functions import f_model_instantiation, Generator, Dataset
...
f_model_x = f_model_instantiation()
q_model = Generator()
...
for epoch in range(num_epochs):
    for _ in range(len(real_dataloader)):
            # Train Generator
            optimizer.zero_grad()
            noise = torch.randn(batch_size, input_size)
            fake_images = q_model(noise)
            fake_outputs = f_model(fake_images)                gen_loss = generator_loss(fake_outputs)
            gen_loss.backward()
            gen_optimizer.step()
    optimizer.zero_grad()
